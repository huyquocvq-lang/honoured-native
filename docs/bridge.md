...
- Observer queries for all nine metrics are registered when the process launches. Immediate background delivery is requested on `SET_AUTH_SESSION`, after `REQUEST_HEALTH_PERMISSION` completes, and at launch when a stored session exists; `CLEAR_AUTH_SESSION` disables it again.
- Each wake first refreshes tracked Live Activities from today's source-deduplicated totals. Native then reads new samples through the anchored queries, writes the batch to the offline queue, and acknowledges HealthKit. Only after that does native attempt **one** upload. If the upload fails, the batch stays queued and is retried on the next wake, on reconnect, or when the app comes to the foreground — so a slow or offline network never blocks HealthKit from waking the app again.
- `.immediate` asks iOS to launch the app whenever HealthKit detects a change; it does not guarantee per-reading delivery. HealthKit may commit device or Apple Watch samples in batches, and iOS decides when to grant background runtime. A wake that arrives while the device is locked before first unlock is deferred until protected data is available.
...
- `enabled` mirrors the "Completion sound" setting, default `true` on a fresh native install. The bundled-gong release also performs a one-time migration from the former `false` default to `true`; subsequent user changes remain persisted. It decides whether the timer and goal notifications carry the gong sound. The in-app gong is played by the web app; native only sets the audio session to `.ambient` so the hardware silent switch is respected.
